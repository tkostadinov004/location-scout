...
function calculate_base_score(r: RentableObject, weights: number[], common_data: ScoreCalculationCommonData): number {
  ...
  return weights[0] * customers_score + weights[1] * rent_score + weights[2] * same_type_objects_score;
}
...
export function calculate_scores(
  ...
  const total_criteria_count = additional_criteria.length + (is_additional_poi_included ? 1 : 0);
  ...
  rentables.forEach((r) => {
    ...
    additional_criteria.forEach((ac) => {
      switch (ac.type) {
        case "park":
          weighted_sum += r.shortest_dist_to_park == null ? 0 : additional_criteria_weight * (1.0 / (1 + r.shortest_dist_to_park));
          break;
        case "public_transport":
          weighted_sum += r.shortest_dist_to_public_transport == null ? 0 : additional_criteria_weight * (1.0 / (1 + r.shortest_dist_to_public_transport));
          break;
        case "school":
          weighted_sum += r.shortest_dist_to_school == null ? 0 : additional_criteria_weight * (1.0 / (1 + r.shortest_dist_to_school));
          break;
      }
    });
    if (is_additional_poi_included) {
      weighted_sum += r.min_distance_to_additional_poi == null ? 0 : additional_criteria_weight * (1.0 / (1 + r.min_distance_to_additional_poi));
    }
    r.total_score = r.base_score == null ? 0 : (r.base_score + weighted_sum) / (1 + additional_criteria_weight * total_criteria_count);
  });
}
